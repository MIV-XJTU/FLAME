...
class LLMTokenizer:
    ...
    def __init__(
        ...
        self.context_length = context_length
        if embedding_method == 'multiprompteol':
            self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_name, **kwargs)
            # if 'llama' in tokenizer_name:
            #     self.tokenizer = LlamaTokenizer.from_pretrained(args.model_name_or_path, use_fast=True)
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_name, add_bos_token=False, add_eos_token=False, **kwargs)
