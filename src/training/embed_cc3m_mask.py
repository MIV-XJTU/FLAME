...
def embed(model, tokenizer, dataloader, args):
    ...
    sample_mode = 'minicpm'
    with open(f'/data/datasets/cc3m/train-flame/flame-long-{sample_mode}-samples_2905954-sorted.json', 'r') as json_file:
    ...
    with open(f'/data/datasets/cc3m/train-flame/flame-short-{sample_mode}-samples_2905954-sorted.json', 'r') as json_file:
    ...
    for i, batch in enumerate(dataloader):
        ...
        for k in range(len(key)):
            num += 1
            save_path = f'/data/datasets/cc3m/train-flame/embeddings/{model.embedding_method}-{model_name}/{key[k][:5]}/{key[k]}.pth'
