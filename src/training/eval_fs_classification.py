...
class MiniImageNetDataset(Dataset):
    def __init__(self, data_root, start, end, visual, mm, llm, tokenizer, llm_reason, tokenizer_reason, preprocess, way=2, shot=1, repeat=0, device="cpu", induction=1):
        ...
        self.tokenizer_reason = tokenizer_reason
        ...
        self.image_ids = []
        self.class_names = []
        with open("/data/mini-imagenet/fewshot_split/%d_way_%d_shot.csv"%(way, shot)) as f:
            for line in f.readlines():
                image_ids = line.strip('\n').split(",")[:-1]
                cur_images = []
                cur_classes = []
                for image_id in image_ids:
                    class_ind, image_ind = image_id.split("/")
                    # path = f'{class_ind}/{class_ind}_{int(str(image_ind[9:-4]))}.JPEG'
                    cur_images.append(image_ind)
        ...
        self.class_names = self.class_names[start: end]
    ...
    def __getitem__(self, index):
        ...
        prompt_tokens = self.tokenizer_reason([instruction], return_tensors='pt', add_special_tokens=False).input_ids.to(self.device)
        in_tokens = self.tokenizer_reason(["###\nInput: < "], return_tensors='pt', add_special_tokens=False).input_ids.to(self.device)
        out_tokens = self.tokenizer_reason([" >\nOutput: "], return_tensors='pt', add_special_tokens=False).input_ids.to(self.device)

        prompt_features = self.llm_reason.model.embed_tokens(prompt_tokens)
        in_feature = self.llm_reason.model.embed_tokens(in_tokens)
        out_feature = self.llm_reason.model.embed_tokens(out_tokens)
        ...
        select_images = torch.cat(select_images, dim=0)
        features = self.visual.get_features(select_images)
        features = self.mm(features)
        _, image_features = self.visual._global_pool(features)
        ...
        # image_features = F.avg_pool2d(patch_features.permute(0, 3, 1, 2), kernel_size=(6, 6), stride=(6, 6), padding=2).permute(0, 2, 3, 1).reshape(bs, 9, embed_dim)
        # image_features = F.avg_pool2d(patch_features.permute(0, 3, 1, 2), kernel_size=(8, 8), stride=(8, 8), padding=1).permute(0, 2, 3, 1).reshape(bs, 4, embed_dim)
        print(image_features.size())

        image_features_head = self.llm.lm_head(image_features).half()
        image_ids = torch.argmax(image_features_head, dim=-1)
        
        words = self.tokenizer.batch_decode(image_ids)
        image_ids = self.tokenizer_reason(words, return_tensors='pt', max_length=16, truncation=True, add_special_tokens=False, padding=True).input_ids.to(self.device)
        ...
        for r in range(0, repeat+1):
            for j in range(len(select_texts)):
                text_ids = self.tokenizer_reason([select_texts[j]], return_tensors='pt', add_special_tokens=False).input_ids.to(self.device)
                text_feature = self.llm_reason.model.embed_tokens(text_ids)

                prompt_tokens = torch.cat([prompt_tokens, in_tokens, image_ids[j].unsqueeze(0), out_tokens, text_ids], dim=1)
                if image_features[j].shape[-1] == prompt_features.shape[-1]:
                    prompt_features = torch.cat([prompt_features, in_feature, image_features[j].unsqueeze(0), out_feature, text_feature], dim=1)

        prompt_tokens = torch.cat([prompt_tokens, in_tokens, image_ids[-1].unsqueeze(0), out_tokens], dim=1)    
        if image_features[-1].shape[-1] == prompt_features.shape[-1]:
            prompt_features = torch.cat([prompt_features, in_feature, image_features[-1].unsqueeze(0), out_feature], dim=1)
        
        out_mask = torch.zeros(16, 32000)
        print(out_mask.size())
        max_len = 0
        for i in range(0, len(query_classes)):
            class_token = self.tokenizer_reason([str(query_classes[i])], return_tensors='pt', add_special_tokens=False).input_ids[0].to(self.device)
        ...
        return prompt_tokens[0], prompt_features[0], target_texts, out_mask
...
def main(args):
    ...
    llm = AutoModelForCausalLM.from_pretrained('mistralai/Mistral-7B-Instruct-v0.3', torch_dtype=torch.float16, trust_remote_code=True).to(device)
    tokenizer = AutoTokenizer.from_pretrained('mistralai/Mistral-7B-Instruct-v0.3', add_bos_token=False, add_eos_token=False, trust_remote_code=True)

    llm_reason = llm
    tokenizer_reason = tokenizer
    ...
    dataset = MiniImageNetDataset(
        data_root='/data/mini-imagenet/miniImagenet/images', 
        start=start,
        end=end, 
        visual=model.visual,
        mm=model.mm, 
    ...
    for i, data in enumerate(tqdm.tqdm(data_loader)):
        prompt_tokens, prompt_features, target_texts, out_mask = data
        min_prompt_len = min(len(t) for t in prompt_tokens)
        max_prompt_len = max(len(t) for t in prompt_tokens)
        total_len = 8 + max_prompt_len
        count = 0
        prediction_ids = []
        with torch.no_grad():
            for cur_pos in range(min_prompt_len, total_len):
                # [N, L, D]
                feature = llm_reason.model(
                    input_ids=prompt_tokens,
                    # inputs_embeds=prompt_features,
                ).last_hidden_state[:, -1, :].unsqueeze(1)
                # print(feature.size())
                # [N, 1, D]
                logit = llm_reason.lm_head(feature)

                if induction:
                    logit = logit * out_mask[:, count, :].to(device)
                prediction_id = torch.argmax(logit, dim=-1)
                # [N, 1]
                if prediction_id[0, 0] == tokenizer_reason.eos_token_id:
                    break

                prompt_tokens = torch.cat([prompt_tokens, prediction_id], dim=1)
                # print(prompt_tokens.size())
                prediction_ids.append(prediction_id[:, :])
                count += 1

            prediction_ids = torch.cat(prediction_ids, dim=1)
            predictions = tokenizer_reason.batch_decode(prediction_ids, skip_special_tokens=True)
        ...
        for target_text, prediction in zip(target_texts, predictions):
            ...
            if pred_text[:len(target_text)] == target_text:
                acc = acc + 1
            print("Prediction: %s \n Ground Truth: %s \n Acc: %.4f (%d / %d) \n"%(pred_text[:len(target_text)], target_text, acc / time, acc, time))
